		...
		*maxNumberOfHElements=originalMaxHElements;
		...
		if (USEARPACK) {
			if (TESTBLANCHARDKAHN)
				...
				maxroots = spacedim+2 ;
		   	if (nroot<=maxroots) {
			  //				Rprintf ("unable to use ARPACK, switching to DGEESX\n") ;
				USEARPACK=0 ;
			}
		}
...
// #endif
			...
			//			Rprintf("done dgees: info = %d, sdim= %d, nroot = %d\n",info,sdim,nroot);
			//			Rprintf("done dgees: rconde = %e, rcondv= %e\n",rconde,rcondv);
...
//#endif
	...
	if (info != 0) {
		Rprintf ("error return from dnaupd, ierr=%d\n", info) ;
		return(-1) ;
	}
...
// #endif
	time_arpack += (cputime() - time0) ; 
	if (info != 0) {
		Rprintf ("error return from dneupd, ierr=%d\n", info) ;
		return(-1) ;
	}
...
#define BKFIXUP 0
	if (BKFIXUP && TESTBLANCHARDKAHN && lowroot <= 1+ZERO_TOL1) {
		...
		Rprintf ("useArpack:  deleting row %d\n", lowpos+1) ;
		...
		if (maxnev-original_maxnev >= 2) {
			Rprintf ("useArpack:  deleting conjugate row %d\n", lowpos+1) ;
			deleteRow (lowpos+1, rootr, maxnev, 1) ;
			deleteRow (lowpos+1, rooti, maxnev, 1) ;
			deleteRow (lowpos+1, spanVecs, maxnev, nroot) ;
		}
...
)
{
	...
	*lfact = (  *maxNumberOfHElements);/*pessimistic setting for filling*/
	...
	for(i=0;i<cColumns;i++){
		...
		bumpSparseAim(qrows);
		if(ierr!=0){Rprintf("*************ran out of space****************\n");return;}
...
int i,j;
for(i=0;i<nrows;i++)
for(j=0;j<ncols;j++)Rprintf("[%d] [%d] %f\n",i,j,matrix[i+(j*nrows)]);
}
...
int i,j,numEls;
numEls=ai[rows]-ai[0];
Rprintf("matrix has %d non zero element/s\n",numEls);
for(i=0;i<rows;i++)
{
for(j=ai[i];j<ai[i+1];j++)
{
Rprintf("row=%d,col=%d,val=%f\n",i+1,aj[j-1],a[j-1]);
}}
}
...
int deleteRow (int targetRow, double *mat, int nrows, int ncols) {
	...
	if (targetRow < 1 || targetRow > nrows) {
		Rprintf ("deleteRow:  target row %d is out of bounds\n", targetRow) ;
		return (-1) ;
	}
